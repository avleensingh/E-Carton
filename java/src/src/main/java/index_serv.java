 ...
 */
@WebServlet(urlPatterns = {"/index_serv"})
@MultipartConfig
   ...
   String bucketName = "bucketName";
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
     ...
     AWSCredentials credentials = new BasicAWSCredentials("Access Key", "Access Key Secret");
            ...
            List<Part> f_parts; // Retrieves filename from input tag
        
        
        f_parts = request.getParts().stream().filter(part -> "file".equals(part.getName())).collect(Collectors.toList());
